...
const PLAN_META: Record<Plan, { name: string; monthlyPrice: number; annualPrice: number; features: string[] }> = {
  solo: {
    name: "Solo",
    monthlyPrice: 17,
    annualPrice: 156,
    features: ["1 Staff Profile", "100 Bookings/mo", "Professional Booking Page", "Website Builder (1 template)", "Booking Confirmations", "Email Support"],
  },
  hub: {
    name: "Hub",
    monthlyPrice: 37,
    annualPrice: 348,
    features: ["Up to 5 Staff Members", "Unlimited Bookings", "Professional Booking Page", "Website Builder (6 templates)", "Automated Email Reminders", "Full Staff Portals", "Booking Confirmations", "Email Support"],
  },
  agency: {
    name: "Agency",
    monthlyPrice: 67,
    annualPrice: 636,
    features: ["Up to 15 Staff Members", "Unlimited Bookings", "Custom Domain Support", "Website Builder (6 templates)", "Automated Email Reminders", "Full Staff Portals", "Advanced Customer Data", "Priority Email Support"],
  },
};
...
function eventLabel(event_type: string) {
  if (event_type === "checkout.session.completed") return "Subscription started";
...
function statusBadge(event_type: string) {
  if (event_type === "invoice.payment_failed") {
    return <span className="inline-flex items-center rounded-full bg-red-50 px-2 py-0.5 text-xs font-semibold text-red-700 border border-red-200">Failed</span>;
  }
  return <span className="inline-flex items-center rounded-full bg-green-50 px-2 py-0.5 text-xs font-semibold text-green-700 border border-green-200">Paid</span>;
}
...
export default function BillingPage() {
  ...
  async function handleUpgrade() {
    ...
    try {
      const res = await fetch("/api/stripe/checkout", {
        ...
        body: JSON.stringify({ plan: selectedPlan, billingCycle: selectedCycle, email: undefined }),
      });
  ...
  const isActive = tenant.tenant_status === "active" && !!tenant.stripe_subscription_id;
  ...
  const availableUpgrades = UPGRADE_ORDER.filter((p) => UPGRADE_ORDER.indexOf(p) > UPGRADE_ORDER.indexOf(plan));
  const availableDowngrades = UPGRADE_ORDER.filter((p) => UPGRADE_ORDER.indexOf(p) < UPGRADE_ORDER.indexOf(plan));

  return (
    <div className="mx-auto max-w-3xl space-y-6 pb-12">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Plan & billing</h1>
        <p className="mt-1 text-gray-500">Manage your subscription and view payment history.</p>
      </div>
      ...
      {(isExpired || (isTrialing && trialDays === 0)) && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 mt-0.5">Choose a plan below to keep access to your account.</p>
          </div>
        </div>
      )}
      ...
      {(isSuspended || isSuspendedStatus) && (
        <div className="rounded-xl border border-orange-200 bg-orange-50 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-orange-800">Your account is suspended</p>
            <p className="text-sm text-orange-700 mt-0.5">Please update your payment method to restore access.</p>
          </div>
        </div>
      )}
      ...
      {isTrialing && trialDays > 0 && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold" style={{ color: trialUrgent ? "#9A3412" : "#065F46" }}>
                {trialDays === 1 ? "Last day of your free trial" : `${trialDays} days left in your free trial`}
              </p>
              <p className="text-sm mt-0.5" style={{ color: trialUrgent ? "#C2410C" : "#047857" }}>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            >
              {meta.name} plan {isTrialing ? "· Trial" : isActive ? "· Active" : ""}
            </span>
            <p className="mt-2 text-2xl font-bold text-gray-900">
              €{meta.monthlyPrice}<span className="text-sm font-normal text-gray-500">/mo</span>
            </p>
            {isActive && (
      ...
      {(!isActive || availableUpgrades.length > 0 || availableDowngrades.length > 0) && (
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {UPGRADE_ORDER.map((p) => {
              const m = PLAN_META[p];
              const price = selectedCycle === "monthly" ? m.monthlyPrice : Math.round(m.annualPrice / 12);
              ...
              return (
                <button
                  key={p}
                  onClick={() => setSelectedPlan(p === plan ? null : p)}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Plan</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {payments.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="px-6 py-3 whitespace-nowrap text-gray-600">{formatDate(p.created_at)}</td>
                    <td className="px-6 py-3 text-gray-800 font-medium">{eventLabel(p.event_type)}</td>
                    <td className="px-6 py-3 text-gray-600 capitalize">
                      {p.plan ? `${p.plan}${p.billing_cycle ? ` · ${p.billing_cycle}` : ""}` : "—"}
                    </td>
