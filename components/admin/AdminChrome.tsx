...
export function AdminChrome({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-4 lg:px-6 lg:py-5">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto overscroll-contain px-3 py-4">
          {NAV.map((item) => (
            <Link
              ...
              href={item.href}
              onClick={() => setMenuOpen(false)}
              className="flex min-h-11 items-center rounded-lg px-3 py-2.5 text-sm text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-900"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="border-t border-gray-100 px-3 py-4 lg:px-6">
          <LogoutButton />
        </div>
      </aside>

      <main className="flex min-h-screen min-w-0 flex-1 flex-col lg:ml-64">
        <header className="sticky top-0 z-20 flex min-h-14 items-center justify-between gap-3 border-b border-gray-100 bg-white px-4 py-2 sm:px-5 lg:hidden">
          <button
            type="button"
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-700 shadow-sm hover:bg-gray-50"
        ...
        <div className="min-w-0 flex-1 overflow-x-hidden px-4 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
